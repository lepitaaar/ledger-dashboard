...
- 출력 규격: `A5 portrait`
- 기준 양식: `거래명세서_양식-2.xlsx`
- 출력 상호명은 계산서 관리 화면에서 선택한 거래처명이다.
- 출력 컬럼: `날짜`, `품명`, `단위`, `수량`, `단가`, `금액`
- 상단 합계금액은 한글 금액과 숫자 금액을 함께 표시한다.
- 하단 `합계금액`, `총계금액`은 출력 품목 금액의 전체 합계다.
- 공급자 정보와 계좌 안내는 서버 환경변수에서 읽는다.
- 다중 페이지 지원:
  - 페이지당 품목 행 수는 18행이다.
  - 데이터가 많으면 다음 페이지로 넘겨 인쇄한다.
  - 각 페이지에 거래처, 공급자, 전체 합계 정보를 반복 출력한다.
